...
(function(){
  ...
  async function checkUpdate(){
    ...
    try{
      const r=await fetch(VERSION_URL+'?whatsnew='+Date.now(),{cache:'no-store',headers:{'Cache-Control':'no-cache'}});
      if(!r.ok) return;
  ...
  async function applyUpdate(){
    const apply=document.getElementById('resumate-ota-apply');
    if(apply){
      apply.disabled=true;
      apply.textContent='Downloading…';
      apply.style.opacity='0.75';
    }
    try{
      if(typeof window.ResuMateOTAUpdateCheck==='function'){
        const started=await window.ResuMateOTAUpdateCheck();
        if(started) return;
      }
      if(window.ResuMateOTA && typeof window.ResuMateOTA.check==='function'){
        const started=await window.ResuMateOTA.check();
        if(started) return;
      }
      throw new Error('OTA updater is not available');
    }catch(e){
      if(apply){
        apply.disabled=false;
        apply.textContent='Download and install now';
        apply.style.opacity='1';
      }
      const msg=String(e && e.message || e || 'Update failed');
      if(typeof window.toast==='function') window.toast('Update failed: '+msg);
      else console.error('ResuMate OTA update failed:',e);
    }
  }
  ...
  function show(){
    ...
    wrap.style.cssText='position:fixed;inset:0;z-index:2147483646;background:rgba(0,0,0,.48);display:flex;align-items:flex-start;justify-content:center;padding:64px 14px 20px;box-sizing:border-box;font-family:system-ui,-apple-system,Segoe UI,sans-serif;pointer-events:auto;touch-action:auto;';
    wrap.innerHTML='<div role="dialog" aria-modal="true" style="width:min(420px,100%);max-height:80vh;overflow:auto;background:#fff;color:#181A1F;border-radius:16px;box-shadow:0 12px 40px rgba(0,0,0,.35);padding:20px;box-sizing:border-box;pointer-events:auto;touch-action:auto;">'
      +'<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;"><strong style="font-size:18px;">What\'s New</strong><button id="resumate-ota-close" type="button" aria-label="Close" style="border:0;background:transparent;font-size:24px;line-height:1;cursor:pointer;touch-action:manipulation;">×</button></div>'
      ...
      +'<button id="resumate-ota-apply" type="button" style="width:100%;border:0;border-radius:10px;background:#181A1F;color:#fff;padding:11px 14px;font:700 14px system-ui,-apple-system,Segoe UI,sans-serif;cursor:pointer;touch-action:manipulation;">Download and install now</button>'
      +'</div>';
    document.body.appendChild(wrap);
    document.getElementById('resumate-ota-close').onclick=close;
    wrap.addEventListener('click',function(e){ if(e.target===wrap) close(); });
    document.getElementById('resumate-ota-apply').onclick=applyUpdate;
  }
...
print('Fixed OTA Download and install now click handling')
